...
#include "funcionesArbolesBinarios.h"
 ...
 void insertarRecursivo(struct NodoArbol *& nodo, NodoGen elemento){
     if(esNodoVacio(nodo))
         ...
         if(elemento.id > nodo->elemento.id)
...
bool buscaArbol(const struct ArbolBinarioBusqueda & arbol, struct NodoGen dato){
    return buscaArbolRecursivo(arbol.arbolBinario.raiz, dato);
}

bool buscaArbolRecursivo(struct NodoArbol * nodo, struct NodoGen dato){
    if(esNodoVacio(nodo)){
        return false;
    }   
    if(nodo->elemento.letra==dato.letra)
        return true;
    if(esNodoVacio(nodo->izquierda)) return false;
    bool izq=buscaArbolRecursivo(nodo->izquierda, dato);
    if(esNodoVacio(nodo->derecha)) return false;
    bool der=buscaArbolRecursivo(nodo->derecha, dato);
    return izq or der;
}
...
struct NodoArbol * eliminarNodoRecursivo(struct NodoArbol * nodo, int elemento){
    ...
    if(elemento < nodo->elemento.id)
        nodo->izquierda = eliminarNodoRecursivo(nodo->izquierda, elemento);
    else if(elemento > nodo->elemento.id)
        ...
        if(esNodoVacio(nodo->izquierda)){
            ...
            nodo->derecha = eliminarNodoRecursivo(nodo->derecha, minNodo->elemento.id);
