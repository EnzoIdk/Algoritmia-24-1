...
void insertarRecursivo(struct NodoArbol *& raiz, int elemento,const struct ArbolBinarioBusqueda &arbol){
    if(esNodoVacio(raiz))
        plantarArbolBB(raiz, nullptr, elemento, nullptr);
    else
        if (raiz->elemento > elemento)
            insertarRecursivo(raiz->izquierda, elemento,arbol);
        else
            if(raiz->elemento < elemento)
                insertarRecursivo(raiz->derecha, elemento,arbol);
            else{
                int max=maximoNodoABB(arbol);
                insertarRecursivo(raiz, elemento+max,arbol);
            }
}

void insertar(struct ArbolBinarioBusqueda & arbol, int elemento){
    insertarRecursivo(arbol.arbolBinario.raiz, elemento,arbol);
}
