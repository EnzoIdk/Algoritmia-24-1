...
struct NodoArbol * crearNuevoNodoArbol(struct NodoArbol * arbolIzquierdo, 
                               NodoGen elemento, struct NodoArbol * arbolDerecho){
...
void imprimeNodo(struct NodoArbol * nodo){
    cout<<setw(5)<<nodo->elemento.fecha<<"-"<<nodo->elemento.cant<<" ";
}
...
int sumarNodosRecursivo(struct NodoArbol * nodo){
    if(esNodoVacio(nodo))
        ...
        return nodo->elemento.fecha + sumarNodosRecursivo(nodo->izquierda) + sumarNodosRecursivo(nodo->derecha);
}
