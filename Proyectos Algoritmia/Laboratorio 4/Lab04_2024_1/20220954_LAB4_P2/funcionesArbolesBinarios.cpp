...
struct NodoArbol * crearNuevoNodoArbol(struct NodoArbol * izquierdo, 
    ...
    nuevoNodo->elemento.relevancia= elemento.relevancia;
    strcpy(nuevoNodo->elemento.titulo,elemento.titulo);
...
void plantarArbolBinario(struct ArbolBinario & arbol, struct NodoArbol * izquierdo,
    ...
    arbol.raiz->tamanio=numeroNodosRecursivo(arbol.raiz);
}
...
int raiz(struct NodoArbol * nodo){
    ...
    return nodo->elemento.relevancia;
}
...
void imprimeNodo(struct NodoArbol * nodo){
    cout<<left<<setw(15)<<nodo->elemento.titulo<<right<<setw(2)<<nodo->elemento.relevancia<<", ";
}
