...
void apilar(struct Pila & pila, struct Ciudad elemento){
    insertarAlInicio(pila.lista, elemento);
    
}

/* pop, elimina un elemento de la parte superior de la pila*/
struct Ciudad desapilar(struct Pila & pila){
	...
	exit(1); //O cualquier otro valor que no se encuentre en la pila
        ...
        struct Ciudad elemento = cima(pila);
...
struct Ciudad cima(const struct Pila & pila){
