...
struct NodoArbol * crearNuevoNodoArbol(struct NodoArbol * arbolIzquierdo, 
                               struct Partido partido, struct NodoArbol * arbolDerecho){
...
void plantarArbolBinario(struct ArbolBinario & arbol, struct NodoArbol * arbolIzquierdo,
    ...
    struct NodoArbol * nuevoNodo = crearNuevoNodoArbol(arbolIzquierdo, partido, arbolDerecho);
    arbol.raiz = nuevoNodo;
}

void plantarArbolBinario(struct ArbolBinario & arbol, struct ArbolBinario arbolIzquierdo,
                         struct Partido partido, struct ArbolBinario arbolDerecho){
    
    struct NodoArbol * nuevoNodo = crearNuevoNodoArbol(arbolIzquierdo.raiz, partido, arbolDerecho.raiz);
    arbol.raiz = nuevoNodo;
}

struct Partido raiz(struct NodoArbol * nodo){
    if (esNodoVacio(nodo)){
...
void imprimeNodo(struct NodoArbol * nodo){
    cout<<setw(5)<<nodo->partido.local<<" ("<<nodo->partido.golesLocal<<") - "<<
            nodo->partido.visitante<<" ("<<nodo->partido.golesVisitante<<")  ";
}
