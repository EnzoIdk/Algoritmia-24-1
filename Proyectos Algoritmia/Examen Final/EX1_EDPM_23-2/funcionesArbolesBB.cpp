...
void plantarArbolBB(struct NodoArbol *& arbol, 
                    struct NodoArbol * arbolIzquierdo, Partido partido, 
...
void insertarRecursivo(struct NodoArbol *& raiz, Partido elemento){
    if(esNodoVacio(raiz))
        plantarArbolBB(raiz, nullptr, elemento, nullptr);
    else{
        char auxR[15],auxP[15];
        strcpy(auxR,raiz->partido.local);
        strcat(auxR,raiz->partido.visitante);
        strcpy(auxP,elemento.local);
        strcat(auxP,elemento.visitante);
        if (strcmp(auxR,auxP)>0)
            insertarRecursivo(raiz->izquierda, elemento);
        else
            if(strcmp(auxR,auxP)<0)
                insertarRecursivo(raiz->derecha, elemento);
            else
                cout<<endl<<"El elemento ya se encuentra en el árbol"<< endl;
    }
}

void insertar(struct ArbolBinarioBusqueda & arbol, Partido elemento){
