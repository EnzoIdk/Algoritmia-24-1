...
void plantarArbolBB(struct NodoArbol *& arbol, 
                    struct NodoArbol * arbolIzquierdo, NodoLetra elemento, 
...
void insertarRecursivo(struct NodoArbol *& raiz, NodoLetra elemento){
    if(esNodoVacio(raiz))
        plantarArbolBB(raiz, nullptr, elemento, nullptr);
    else
        if (raiz->elemento.orden > elemento.orden)
            insertarRecursivo(raiz->izquierda, elemento);
        else
            if(raiz->elemento.orden < elemento.orden)
                insertarRecursivo(raiz->derecha, elemento);
            else
                cout<<endl<<"El elemento "<< elemento.letra <<" ya se encuentra en el árbol"<< endl;
}

void insertar(struct ArbolBinarioBusqueda & arbol, NodoLetra elemento){
...
//
//NodoLetra minimoNodoABB(const struct ArbolBinarioBusqueda & arbolBB){
//    return minimoNodo(arbolBB.arbolBinario.raiz);
//}
//NodoLetra maximoNodoABB(const struct ArbolBinarioBusqueda & arbolBB){
//    return maximoNodo(arbolBB.arbolBinario.raiz);
//}
//
//NodoLetra minimoNodo(struct NodoArbol * nodo){
...
//NodoLetra maximoNodo(struct NodoArbol * nodo){
//    if(esNodoVacio(nodo)){
//        return -1;
//    }
