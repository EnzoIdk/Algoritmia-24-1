...
void insertarencola(struct Lista & tad, int detector, int elemento) {
    struct Nodo * nuevoNodo = crearNodo(elemento, detector, nullptr);

    if (esListaVacia(tad)){/*Si la lista está vacia*/
        tad.cabeza = nuevoNodo; /*se inserta en la cabeza de la lista*/
        tad.cola = nuevoNodo;
        if(detector==2) tad.finSistema=nuevoNodo;
        else tad.finSistema=nullptr;
    } 
    else{ //La lista ya tiene nodos
        if(detector==2){
            if(tad.finSistema==nullptr){
                tad.finSistema=nuevoNodo;
                tad.finSistema->siguiente=tad.cabeza;
                tad.cabeza=tad.finSistema;
            }
            else{
                //SI ES EL UNICO ELEMENTO DE SU TIPO, DEBEMOS ACTUALIZAR LA COLA
                if(tad.finSistema->siguiente==nullptr) tad.cola=nuevoNodo;
                //AGREGAR NORMAL
                nuevoNodo->siguiente=tad.finSistema->siguiente;
                tad.finSistema->siguiente=nuevoNodo;
                tad.finSistema=nuevoNodo;
            }
        }
        else{
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout <<"("<<recorrido->elemento<<" : "<<recorrido->detector<<")";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
