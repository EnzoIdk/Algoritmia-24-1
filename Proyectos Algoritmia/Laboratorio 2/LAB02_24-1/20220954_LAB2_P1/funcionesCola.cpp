...
void encolar(struct Cola & cola, int dd,int mm,int aa,const char *codigo){
    int fecha=aa*10000+mm*100+dd;
    int edad=(FECHA_ACTUAL-fecha)/10000;
    int prioridad=1;
    if(edad>80) prioridad=3;
    else if(edad<10) prioridad=2;
    insertarencola(cola.lista, fecha,codigo,prioridad);
