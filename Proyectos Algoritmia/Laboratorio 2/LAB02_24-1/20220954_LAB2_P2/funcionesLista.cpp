...
void insertarAlInicio(struct Lista & tad, int elemento,int peso) {
...
void insertarAlFinal(struct Lista & tad, int elemento,int peso) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(tad);
    struct Nodo * nuevoNodo = crearNodo(elemento,peso,nullptr);
...
struct Nodo * crearNodo(int elemento, int peso,struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
    nuevoNodo->peso=peso;
...
int retornaPeso(const struct Lista &tad){
    return tad.cabeza->peso;
}
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento<<'-'<<recorrido->peso;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
