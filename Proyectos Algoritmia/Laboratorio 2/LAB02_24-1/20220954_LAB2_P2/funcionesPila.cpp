...
void apilar(struct Pila & pila, int elemento,int peso){
    insertarAlInicio(pila.lista, elemento,peso);
...
int desapilarPeso(struct Pila & pila){
...
int cimaP(const struct Pila & pila){
    
    if (esPilaVacia(pila)){
//	cout<<"La pila está vacía por lo tanto no posee cima"<<endl;
        return -1; //O cualquier otro valor que no se encuentre en la pila
    }		
    return retornaPeso(pila.lista);
}
