...
void insertarAlInicio(struct Lista & tad, int nivel,char tipo) {
...
void insertarAlFinal(struct Lista & tad, int nivel,char tipo) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(tad);
    struct Nodo * nuevoNodo = crearNodo(nivel,tipo, nullptr);
...
struct Nodo * crearNodo(int nivel,char tipo, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->nivel= nivel;
    nuevoNodo->tipo=tipo;
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << '('<<recorrido->nivel<<'-'<<recorrido->tipo<<')';
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
